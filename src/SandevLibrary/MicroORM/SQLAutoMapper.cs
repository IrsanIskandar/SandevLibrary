...
public static class SQLAutoMapper
{
	public static async Task<T> ToSingleAsync<T>(this DbDataReader dataReader)
	{
		...
		Dictionary<string, PropertyInfo> returnObjectProperties = returnType.GetProperties(BindingFlags.Instance | BindingFlags.Public)
		...
		for (int i = 0; i < dataReader.FieldCount; i++)
		{
			...
			{
				PropertyInfo propertyInfo = returnObjectProperties[colName];

				if (propertyInfo != null && propertyInfo.CanWrite)
				{
					object colValue = dataReader.GetValue(i);
					bool isNull = string.IsNullOrEmpty(colValue.ToString());
