...
namespace SandevLibrary.SecurityAlgorithm
{
    public class BlowFishAlgorithm
    {
        ...
        private static readonly int ROUNDS = 16; //standard is 16, to increase the number of rounds, bf_P needs to be equal to the number of rouds. Use digits of PI.

        //Random number generator for creating IVs
        private static RNGCryptoServiceProvider randomSource;

        //SBLOCKS
        private static uint[] bf_s0;
        private static uint[] bf_s1;
        private static uint[] bf_s2;
        private static uint[] bf_s3;
        ...
        public BlowFishAlgorithm(string hexKey)
        {
        ...
        public BlowFishAlgorithm(byte[] cipherKey)
        {
            randomSource = new RNGCryptoServiceProvider();
            SetupKey(cipherKey);
        }
        ...
        public string Encrypt_CBC(string plaintext)
        {
            if (!IVSet)
                SetRandomIV();
            return ByteToHex(InitVector) + ByteToHex(Encrypt_CBC(Encoding.ASCII.GetBytes(plaintext)));
        }
        ...
        {
            IV = HexToByte(ciphertext.Substring(0, 16));
            return Encoding.ASCII.GetString(Decrypt_CBC(HexToByte(ciphertext.Substring(16)))).Replace("\0", "");
        }
        ...
        /// <returns>Plaintext</returns>
        private static byte[] Decrypt_CBC(byte[] ciphertext)
        {
        ...
        private static byte[] Encrypt_CBC(byte[] pt)
        {
        ...
        {
            return ByteToHex(Encrypt_ECB(Encoding.ASCII.GetBytes(Plaintext)));
        }
        ...
        private static byte[] Encrypt_ECB(byte[] pt)
        {
        ...
        /// <returns>Plaintext</returns>
        private static byte[] Decrypt_ECB(byte[] ct)
        {
        ...
        public static byte[] SetRandomIV()
        {
        ...
        private static void SetupKey(byte[] cipherKey)
        {
            bf_P = SetupP();
            //set up the S blocks
            bf_s0 = SetupS0();
        ...
        private static byte[] Crypt_ECB(byte[] text, bool decrypt)
        {
        ...
        private static byte[] Crypt_CTR(byte[] text, int numThreads)
        {
            if (!IVSet)
            {
        ...
        /// <param name="decrypt">true to decrypt, false to encrypt</param>
        /// <returns>(En/De)crypted data</returns>
        private static byte[] Crypt_CBC(byte[] text, bool decrypt)
        {
            if (!IVSet)
            {
                throw new Exception("IV not set.");
        ...
        private static void BlockEncrypt(ref byte[] block)
        {
            SetBlock(block);
            encipher();
            GetBlock(ref block);
        ...
        private static void BlockDecrypt(ref byte[] block)
        {
            SetBlock(block);
            decipher();
            GetBlock(ref block);
        ...
        private static void SetBlock(byte[] block)
        {
        ...
        private static void GetBlock(ref byte[] block)
        {
            byte[] block1 = new byte[4];
            byte[] block2 = new byte[4];
            if (nonStandardMethod)
        ...
        /// <returns></returns>
        private static uint round(uint a, uint b, uint n)
        {
